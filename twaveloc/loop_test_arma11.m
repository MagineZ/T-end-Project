...
for snr = [10,5]
    ...
    save(['loop_test_arma11' int2str(snr) 'dB.mat'])
    Table_ar1 = [Table_ar1;nanmean(meZ) ,nanmean(sdZ), nanmean(meZ_sh) ,nanmean(sdZ_sh), nanmean(meC), nanmean(sdC), nanmean(meC_sh), nanmean(sdC_sh),nanmean(meW), nanmean(sdW),nanmean(meW_sh), nanmean(sdW_sh)];
end
