...
for snr = [10,5]
    ...
    save(['loop_test2_arma11' int2str(snr) 'dB.mat'])
    Table_ar2 = [Table_ar2;nanmean(meZ(:,1)) ,nanmean(sdZ(:,1)), nanmean(meZ_sh(:,1)) ,nanmean(sdZ_sh(:,1)), nanmean(meC(:,1)), nanmean(sdC(:,1)), nanmean(meC_sh(:,1)), nanmean(sdC_sh(:,1)),nanmean(meW(:,1)), nanmean(sdW(:,1)),nanmean(meW_sh(:,1)), nanmean(sdW_sh(:,1))];
    Table_ar3 = [Table_ar3;nanmean(meZ(:,2)) ,nanmean(sdZ(:,2)), nanmean(meZ_sh(:,2)) ,nanmean(sdZ_sh(:,2)), nanmean(meC(:,2)), nanmean(sdC(:,2)), nanmean(meC_sh(:,2)), nanmean(sdC_sh(:,2)),nanmean(meW(:,2)), nanmean(sdW(:,2)),nanmean(meW_sh(:,2)), nanmean(sdW_sh(:,2))];
end
