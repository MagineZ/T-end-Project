function [tfr, ceps, tceps, tfrr, rtfr, tfrsq, tfrtic, t] = CFPH(x, basicTF, advTF, cepR, P,h)
...
% [h,Dh,tt] = hermf(win,2,6) ; h = h'; Dh = Dh';
...
% a = lpc(x,150);
% est_x = filter(a,1,x);
% [tfr, tfrtic, tfrsq, tfrsqtic] = sqSTFTbase(x, 0, 0.5, 0.0005, 1, h2, Dh2) ;
% [tfr, tfrtic, tfrsq, tfrsqtic] = sqSTFTbase(x, 0, 0.5, 0.02/20, 10, h2, Dh2);
...
if g~=0
    ...
    %TMP = ifft(abs(tfr).^g, 2*size(tfr,1), 1) ; 
    %TMP = TMP - repmat(mean(TMP,1),size(TMP,1), 1) ;
    %ceps = 2.*real(TMP);
