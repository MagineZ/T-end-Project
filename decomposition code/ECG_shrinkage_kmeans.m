function  [Om0,Om0_real] = ECG_shrinkage_kmeans( x0,x0_real, current_beats, current_beats_q, OptimalShrinkageOpt,num_nonlocal, ifauto)

% optimal shrinakge of ECG

RRI = diff(current_beats);
RRI = [RRI(1) RRI];
x0_RS = current_beats_q-current_beats;
R_amp = x0(current_beats);
S_amp = x0(current_beats_q);

%MaximalQT = ceil(max(RRI)/2);
MaximalQTp = ceil((median(RRI)+2*std(RRI))*3/8);
MaximalQTt = ceil((median(RRI)+2*std(RRI))*5/8);
...
for ii = 1:length(current_beats)
    ...
    R(count) = x0(current_beats(ii));
    S(count) = x0(current_beats_q(ii));
    tRS(count) = current_beats_q(ii) - current_beats(ii);
    II = [II;idx];
...
for s = 1:length(current_beats)
    ...
    %Z = abs(R_amp - R_amp(s))+abs(S_amp - S_amp(s));
    Z = abs(RRI - RRI(s));
    
    Z(s) = inf;
    [~,ind] = sort(Z,'ascend');
    Nidx = [s, ind(1:min(4*num_nonlocal,length(Z)-1))];
    ...
    [~,ind] = sort(ZZZ(Nidx),'ascend');
    
    Nidx =[s,Nidx(ind(1:(num_nonlocal-1)))];
    XN = V(:,Nidx);
    XN2 = V2(:,Nidx);
