function  [Om0,Om0_real] = ECG_shrinkage0( x0,x0_real, current_beats, OptimalShrinkageOpt, sigma_coeff,ifauto)
...
if n_theta>n_t
    ...
    covM = (V-u0*ones(1,n_theta))*(V-u0*ones(1,n_theta))';
    beta0 = n_t/n_theta;
    [ae,be,ce] = eig(covM./((sigma^2)*n_theta)) ;
    lambdaOSe = diag(be) ;
    if ifauto
        eigvals = optimal_shrinkage_pca(lambdaOSe,beta0,OptimalShrinkageOpt);
    else
        eigvals = optimal_shrinkage_pca(lambdaOSe,beta0,OptimalShrinkageOpt,1);
    end
    %covM = (sigma^2)*n_theta.*(ae*diag(eigvals)*ce(:,1:n_t)');
    %XN0 = u0*ones(1,n_theta)+covM*(V-u0*ones(1,n_theta));
    r_ev = sqrt(pinv(be))*ae'*(V-u0*ones(1,n_theta));
    XN0 = u0*ones(1,n_theta) + ae*sqrt(diag(eigvals))*r_ev;
else
   
    covM = (V-u0*ones(1,n_theta))'*(V-u0*ones(1,n_theta));
    beta0 = n_theta/n_t;
    [ae,be,ce] = eig(covM./((sigma^2)*n_t)) ;
    lambdaOSe = diag(be) ;
    ...
    %covM = (sigma^2)*n_t.*(ae*diag(eigvals)*ce(:,1:n_theta)');
    %XN0 = u0*ones(1,n_theta)+(V-u0*ones(1,n_theta))*covM;
    r_ev = sqrt(pinv(be))*ae'*(V-u0*ones(1,n_theta))';
    XN0 = u0*ones(1,n_theta) + (ae*sqrt(diag(eigvals))*r_ev)';
end
...
if n_theta>n_t
    ...
    covM2 = (V2-u0*ones(1,n_theta))*(V2-u0*ones(1,n_theta))';
    beta0 = n_t/n_theta;
    [ae,be,ce] = eig(covM2./((sigma^2)*n_theta)) ;
    lambdaOSe = diag(be) ;
    ...
    %covM2 = (sigma^2)*n_theta.*(ae*diag(eigvals)*ce(:,1:n_t)');
    %XN02 = u0*ones(1,n_theta)+covM2*(V-u0*ones(1,n_theta));
    r_ev = sqrt(pinv(be))*ae'*(V2-u0*ones(1,n_theta));
    XN02 = u0*ones(1,n_theta) + ae*sqrt(diag(eigvals))*r_ev;
else
    
    covM2 = (V2-u0*ones(1,n_theta))'*(V2-u0*ones(1,n_theta));
    beta0 = n_theta/n_t;
    [ae,be,ce] = eig(covM2./((sigma^2)*n_t)) ;
    lambdaOSe = diag(be) ;
    if ifauto
        eigvals = optimal_shrinkage_pca(lambdaOSe,beta0,OptimalShrinkageOpt);
    else
    ...
    %covM2 = (sigma^2)*n_t.*(ae*diag(eigvals)*ce(:,1:n_theta)');
    %XN02 = u0*ones(1,n_theta)+(V-u0*ones(1,n_theta))*covM2;
    r_ev = sqrt(pinv(be))*ae'*(V2-u0*ones(1,n_theta))';
    XN02 = u0*ones(1,n_theta) + (ae*sqrt(diag(eigvals))*r_ev)';
end
