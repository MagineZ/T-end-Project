function [position,positionaux,messages] = wavedet_3D_pcs(sig_all, ext_anot,fs )
ft = 2;
str = '1';
positionaux=[];
position=[];
...
messages.setup.wavedet.QRS_detection_only=false;
messages.setup.wavedet.QRS_detection_thr=ones(5,1);
messages.errors=[];
...
while ((endsamp+1) < t(2))
    ...
    if isnan(ft)||ft==0||ft==1||ft==2||ft==4||ft>4
        sig=sig_all(inisamp-t(1)+1:min(endsamp-t(1)+1,size(sig_all,1)),1);
    end
